...
//                uri.getScheme() != null ? uri.getScheme() : "http");
        ...
        System.out.println("Reverse proxy on " +  port);
    ...
    static class ProxyRequestProducer implements HttpAsyncRequestProducer {
        ...
        public HttpRequest generateRequest() {
            synchronized (this.httpExchange) {
                ...
                String proxyHost = request.getHeaders("Host")[0].toString();

                if (uri.startsWith("/?url=")) uri = uri.substring(6);
                if (uri.startsWith("/?")) uri = uri.substring(2);
                if (uri.startsWith("/")) uri = uri.substring(1);

                System.err.println("  loading " + uri + " via " + proxyHost);
                ...
                try {
                    ...
                    HttpHost target = new HttpHost(u.getHost(), p);
                    httpExchange.setTarget(target);

                    System.out.println("Loading: " + u.getPath());
                    System.out.println("Loading: " + httpExchange.getRequest());
                ...
                return new BasicHttpRequest("GET", u.getPath());
