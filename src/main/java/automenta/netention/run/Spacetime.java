...
public class Spacetime extends Application {
    ...
    public final HashSet<Object> singletons = new HashSet<Object>();
...
//    }
    ...
    public static class API {
        ...
        public Object get() {
            try {
                List<RESTEndpoints.Endpoint> x = RESTEndpoints.restEndpoints(Spacetime.class);
     ...
     * http://dalelane.co.uk/blog/?p=1871
     */
    public static class RESTEndpoints {
        ...
        public static void forEachEndpoint(Class clazz, Consumer<Endpoint> with) {
            ...
            if (annotation != null) {
                ...
                for (Method method : methods) {
                    if (method.isAnnotationPresent(GET.class)){
                        with.accept(createEndpoint(method, MethodEnum.GET, clazz, basePath));
                    }
                    else if (method.isAnnotationPresent(PUT.class)){
                        with.accept(createEndpoint(method, MethodEnum.PUT, clazz, basePath));
                    }
                    else if (method.isAnnotationPresent(POST.class)){
                        with.accept(createEndpoint(method, MethodEnum.POST, clazz, basePath));
                    }
                    else if (method.isAnnotationPresent(DELETE.class)){
                        with.accept(createEndpoint(method, MethodEnum.DELETE, clazz, basePath));
                    }
                }
            }
        ...
        public static List<Endpoint> restEndpoints(String basepackage) throws IOException, ClassNotFoundException {
            List<Endpoint> endpoints = new ArrayList<Endpoint>();

            List<Class> classes = getClasses(basepackage);

            for (Class<?> clazz : classes) {
                forEachEndpoint(clazz, e -> endpoints.add(e));
            }
         ...
         */
        @SuppressWarnings("rawtypes")
        public static List<Endpoint> restEndpoints(Class... classes) throws IOException, ClassNotFoundException {
            List<Endpoint> endpoints = new ArrayList<Endpoint>();
         ...
         */
        private static Endpoint createEndpoint(Method javaMethod, MethodEnum restMethod, Class<?> clazz, String classUri){
         ...
         */
        @SuppressWarnings("rawtypes")
        private static void discoverParameters(Method method, Endpoint endpoint){
...
//                }
         ...
         */
        @SuppressWarnings("rawtypes")
        private static List<Class> getClasses(String pkg) throws IOException, ClassNotFoundException {
        ...
        public static class Endpoint implements Serializable {
            ...
            public List<EndpointParameter> queryParameters = new ArrayList<>();
            public List<EndpointParameter> pathParameters = new ArrayList<>();
            public List<EndpointParameter> payloadParameters = new ArrayList<>();
        }

        public static class EndpointParameter implements Serializable {
            public ParameterType parameterType = ParameterType.PAYLOAD;
    ...
    public static UndertowJaxrsServer start(Undertow.Builder builder) {
        UndertowJaxrsServer server = new UndertowJaxrsServer().start(builder);
        server.deploy(Spacetime.class);
        return server;
    }
...
//        DeploymentInfo di = server.undertowDeployment(Spacetime.class);
//        System.out.println (di);
//        ServletInfo res = ((ServletInfo) di.getServlets().get("ResteasyServlet"));
//
//        System.out.println( res.getClass() );
//        System.out.println( res.getMappings() );
//
//
//        System.out.println(di.getListeners());
//        System.out.println (di.getResourceManager() );
