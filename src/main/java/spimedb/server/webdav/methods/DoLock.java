...
public class DoLock extends AbstractMethod {
    ...
    private void doLock(ITransaction transaction, HttpServletRequest req,
        ...
        StoredObject so = _store.get(transaction, _path);
    ...
    private void doNullResourceLock(ITransaction transaction,
        ...
        try {
            parentSo = _store.get(transaction, _parentPath);
            if (_parentPath != null && parentSo == null) {
                _store.newFolder(transaction, _parentPath);
            ...
            nullSo = _store.get(transaction, _path);
            if (nullSo == null) {
                // resource doesn't exist
                _store.newResource(transaction, _path);
     ...
     */
    private static int getTimeout(ITransaction transaction, HttpServletRequest req) {
