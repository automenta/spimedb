...
public class DoHead extends AbstractMethod {
    ...
    public void run(ITransaction transaction, HttpServletRequest req,
        ...
        if (so == null) {
            if (this._insteadOf404 != null && !_insteadOf404.trim().isEmpty()) {
                path = this._insteadOf404;
                so = _store.get(transaction, this._insteadOf404);
            }
        } else
            bUriExists = true;
        ...
        if (so != null) {
            if (so.isFolder()) {
                if (_dftIndexFile != null && !_dftIndexFile.trim().isEmpty()) {
                ...
                try {
                    ...
                    if (so.isResource()) {
                        ...
                        if (path.endsWith("/") || (path.endsWith("\\"))) {
                            ...
                            String mimeType = _mimeTyper.mimeType(path);
                            if (mimeType != null) {
