	...
	Tree.prototype.clickHandler = function (event) {
		...
		if ((classList.indexOf('click-expand') != -1) ||
			...
			if (node.label == 'section') {
				$('#treeview').treeview('toggleNodeExpanded', [ node.nodeId, { levels: 2, silent: true } ]);
			}
		}
    }
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			...
			var treeItem = $(_this.template.item)
				...
				.attr('data-details', node.details)
				.attr('data-3d', node.only3d)
				.attr('data-showtree', node.showTree)
				.attr('data-zoom', node.zoom);
...
//	Tree.prototype.css = '.treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:0;margin-right:0}.treeview span.expand-collapse{width:1rem;height:1rem}.treeview span.icon{margin-left:10px;margin-right:5px}'
