...
import static spimedb.media.KML.anchorHash;
 ...
 * calling {@link
 *  Feature#getElements() getElements()} on the
 * feature.
 * <p/>
 * </li>
...
public class KmlInputStream extends XmlInputStream implements IKml {
    ...
    public static List<Point> parseCoord(String coord) {
        ...
        if (numparts != 0 && lon != COORD_ERROR) {
            switch (numparts) {
                case 1 -> list.add(new Point(new Geodetic2DPoint(lon, new Latitude())));
                case 2 -> list.add(new Point(new Geodetic2DPoint(lon, lat)));
                case 3 -> list.add(new Point(new Geodetic3DPoint(lon, lat, elev)));
            }
        }
...
//            }
    ...
    private IGISObject handleContainer(XMLEvent e) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (ne.getEventType() == XMLStreamConstants.START_ELEMENT) {
            ...
            if (ee.getEventType() == XMLStreamConstants.START_ELEMENT) {
    ...
    private void handleExtendedData(Common cs, QName name)
        ...
        while (true) {
            ...
            if (next.getEventType() == XMLStreamConstants.START_ELEMENT) {
    ...
    private void handleStyleMapPair(StyleMap sm, QName name) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (ce.getEventType() == XMLStreamConstants.START_ELEMENT) {
    ...
    private Style handleStyle(Common cs, XMLEvent ee, QName name) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (next.getEventType() == XMLStreamConstants.START_ELEMENT) {
                ...
                switch (localPart) {
                    case ICON_STYLE -> handleIconStyle(style, qname);
                    case LINE_STYLE -> handleLineStyle(style, qname);
                    case BALLOON_STYLE -> handleBalloonStyle(style, qname);
                    case LABEL_STYLE -> handleLabelStyle(style, qname);
                    case POLY_STYLE -> handlePolyStyle(style, qname);
                    case LIST_STYLE -> handleListStyle(style, qname);
                }
            }
        }
    }
    ...
    private void handlePolyStyle(Style style, QName qname) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (e.getEventType() == XMLStreamConstants.START_ELEMENT) {
                ...
                String localPart = se.getName().getLocalPart();
                switch (localPart) {
                    case FILL -> fill = isTrue(stream.getElementText()); // default=true
                    case OUTLINE -> outline = isTrue(stream.getElementText()); // default=true
                    case COLOR -> color = parseColor(stream.getElementText()); // default=WHITE
                }
            }
        }
    }
        ...
        while (true) {
            ...
            if (e.getEventType() == XMLStreamConstants.START_ELEMENT) {
                ...
                switch (name) {
                    ...
                    case BG_COLOR -> color = parseColor(stream.getElementText());
                    case DISPLAY_MODE -> displayMode = getNonEmptyElementText();
                    case TEXT_COLOR -> textColor = parseColor(stream.getElementText());
                    case COLOR ->
    ...
    private IGISObject handleStartElement(XMLEvent e) throws XMLStreamException {
        ...
        try {
            ...
            log.warn("Skip element: {} {}", localname, e1);
            skipNextElement(stream, name);
        }
        /*
            ...
            if (next.getEventType() == XMLStreamConstants.START_ELEMENT) {
                ...
                if (updateFlag) {
                    switch (tag) {
                        ...
                        case "Create" -> c.setUpdateType("Create");
                        case "Delete" -> c.setUpdateType("Delete");
                        case "Change" -> c.setUpdateType("Change");
                    }
                } else {
                    switch (tag) {
                        ...
                        case "Update" -> updateFlag = true; // set flag to parse inside Update element
                    }
                }
            } else {
                    ...
                    if (type != null) {
                        ...
                        if (StringUtils.isNotBlank(typeValue) && !"wstring".equalsIgnoreCase(typeValue)) {
                            try {
                                ...
                                log.warn("Invalid SimpleField type in field [name={} type={}]", fieldname, typeValue);
     ...
     */
    @Nullable
    private static ScreenLocation handleScreenLocation(StartElement sl) {
        try {
            ...
            if (event.getEventType() == XMLStreamConstants.START_ELEMENT) {
                ...
                if (value != null) {
                    try {
                        ...
                        log.error("Invalid GroundOverlay angle {} in {}", value, sename);
                    } catch (IllegalArgumentException nfe) {
...
@NonNull
     ...
     */
    @Nullable
    private static IGISObject handleEndElement(XMLEvent e) {
