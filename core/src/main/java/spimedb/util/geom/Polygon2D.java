...
public class Polygon2D implements Shape2D, Iterable<Vec2D> {
    ...
    protected static boolean intersectsLine(Line2D l, List<Line2D> edges) {
        for (Line2D e : edges) {
            final Line2D.LineIntersection.Type isec = l.intersectLine(e).getType();
            if (isec == Line2D.LineIntersection.Type.INTERSECTING || isec == Line2D.LineIntersection.Type.COINCIDENT) {
                return true;
            }
        }
        return false;
    ...
    public boolean toOutline() {
        ...
        for (i = 0; i < segs - 1; i++) {
            for (j = i + 1; j < segs; j++) {
                ...
                Line2D.LineIntersection isec = li.intersectLine(lj);
