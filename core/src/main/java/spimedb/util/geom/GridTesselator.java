...
import spimedb.util.geom.mesh2d.DelaunayTriangulation;
import spimedb.util.geom.mesh2d.Voronoi;
...
public abstract class GridTesselator implements PolygonTesselator {
    ...
    public List<Triangle2D> tesselatePolygon(Polygon2D poly) {
        ...
        for (Triangle2D t : voronoi.getTriangles()) {
            if (Math.abs(t.a.x) != Voronoi.DEFAULT_SIZE
                    && Math.abs(t.a.y) != Voronoi.DEFAULT_SIZE) {
