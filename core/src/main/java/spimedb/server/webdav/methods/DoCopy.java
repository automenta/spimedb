...
import spimedb.server.webdav.exceptions.*;
import spimedb.server.webdav.fromcatalina.RequestUtil;
...
public class DoCopy extends AbstractMethod {
    ...
    public boolean copyResource(ITransaction transaction,
        ...
        if (_resourceLocks.lock(transaction, destinationPath, lockOwner, false,
            ...
            try {
                copySo = _store.get(transaction, path);
                ...
                destinationSo = _store.get(transaction,
                        destinationPath);
    ...
    private void copy(ITransaction transaction, String sourcePath,
        ...
        StoredObject sourceSo = _store.get(transaction, sourcePath);
        if (sourceSo.isResource()) {
            _store.newResource(transaction, destinationPath);
            long resourceLength = _store.put(transaction,
                    destinationPath, _store.stream(transaction,
                            sourcePath), null, null);

            if (resourceLength != -1) {
                StoredObject destinationSo = _store.get(
    ...
    private void copyFolder(ITransaction transaction, String sourcePath,
        ...
        _store.newFolder(transaction, destinationPath);
        ...
        if (infiniteDepth) {
            ...
            for (int i = children.length - 1; i >= 0; i--) {
                ...
                try {
                    childSo = _store.get(transaction,
                            (sourcePath + children[i]));
                    if (childSo.isResource()) {
                        _store.newResource(transaction, destinationPath
                                + children[i]);
                        long resourceLength = _store.put(
                                transaction, destinationPath + children[i],
                                _store.stream(transaction,
     ...
     */
    private static String parseDestinationHeader(HttpServletRequest req,
